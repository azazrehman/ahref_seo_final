...
module.exports = {
  ...
  click: async function (page, selector) {
    try {
      ...
      throw new Error(
        `Could not click on selector: ${selector}  Detail Error:` + error
      );
    }
  },
    ...
    try {
      await Promise.all([
        ...
        page.waitForNavigation({
          waitUntil: waits.networks.NETWORK_IDEAL_0 /*,timeout:0*/,
        }),
      ]);
    } catch (error) {
      ...
      if (selector.startsWith("//") || selector.startsWith("(//")) {
        ...
        await page.waitForSelector(selector, {
          ...
          await Promise.all([
            ...
            throw new Error(
              `Could not type text into selector: ${selector} -> ${error}`
            );
          });
   ...
   */
  click_xPath: async function (page, selector) {
    //TODO remove all click_xPath explicitly calls
    try {
   ...
   * @return {Void} Nothing
   */
  waitForSelector: async function (
    page,
    selector,
    timeout = config.waitingTimeout
  ) {
